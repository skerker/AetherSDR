...
typedef struct _buffer_descriptor
{
	...
	//! host-order VITA header for packet timing/sequence diagnostics
	uint32 vita_header;
	//! host-order four-bit VITA packet count
	uint32 vita_packet_count;
	//! missing VITA packets detected immediately before this buffer
	uint32 vita_missing_packets;
	//! local monotonic receive time, used to measure waveform turnaround
	uint64 arrival_monotonic_ns;
	//! caller-owned correlation identifier for asynchronous processing
